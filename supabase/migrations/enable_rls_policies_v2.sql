...
-- 1. Enable RLS on existing tables (based on your screenshot)
...
-- Repair Requests: Anyone can read (needed for review page access by token)
...
-- Repair Request Services: Anyone can insert (when creating repair request)
CREATE POLICY "Anyone can create repair request services"
ON repair_request_services FOR INSERT
WITH CHECK (true);
...
-- 5. VIEWS (if they exist, enable RLS)
-- Note: Views shown in your screenshot
DO $$
BEGIN
    -- Check if daily_expense_stats view exists
    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'daily_expense_stats') THEN
        ALTER VIEW daily_expense_stats OWNER TO authenticated;
    END IF;

    -- Check if daily_revenue_stats view exists
    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'daily_revenue_stats') THEN
        ALTER VIEW daily_revenue_stats OWNER TO authenticated;
    END IF;

    -- Check if other views exist
    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'expense_by_category') THEN
        ALTER VIEW expense_by_category OWNER TO authenticated;
    END IF;

    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'monthly_expense_stats') THEN
        ALTER VIEW monthly_expense_stats OWNER TO authenticated;
    END IF;

    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'monthly_profit_stats') THEN
        ALTER VIEW monthly_profit_stats OWNER TO authenticated;
    END IF;

    IF EXISTS (SELECT 1 FROM pg_views WHERE viewname = 'monthly_revenue_stats') THEN
        ALTER VIEW monthly_revenue_stats OWNER TO authenticated;
    END IF;
END $$;
